...
class GeneExpressionDataset(Dataset):
    ...
    def __init__(self, adata, target_gene_mapping, batch_mapping,batch_var_dict,sampleCount:int = 700,target_gene_dim:int = 128):
        ...
        """
        # Store data sources
        #mask = (adata.obs.target_gene != "non-targeting").to_numpy()
...
def get_loader(num_samples,target_gene_dims = 128):
    adata,maskidx = load_data()
    gene_dim,batch_dim,target_gene_mapping,batch_mapping = get_mappings(adata)
    dataset = GeneExpressionDataset(adata, target_gene_mapping, batch_mapping,maskidx,sampleCount = num_samples,target_gene_dim=target_gene_dims)
    np.save('highvars.npy',maskidx)
    return dataset,gene_dim,maskidx
